class Stacker {
  ...
  private goldPos: { x: number; y: number; type: number } = {
    x: -1,
    y: -1,
    type: -1,
  };

  constructor(agentX: number, agentY: number) {
  ...
  private updateGoldPosition(
    currentPos: { x: number; y: number },
    gameMap: number[][]
  ): void {
    const visibilityRange = 5;
    for (
      let y = Math.max(0, currentPos.y - visibilityRange);
      y <= Math.min(gameMap.length - 1, currentPos.y + visibilityRange);
      y++
    ) {
      for (
        let x = Math.max(0, currentPos.x - visibilityRange);
        x <= Math.min(gameMap[0].length - 1, currentPos.x + visibilityRange);
        x++
      ) {
        if (
          gameMap[y][x] === this.GOLD &&
  ...
  private decideDropBlock(): string {
  ...
  private findNearestBlock(gameMap: number[][]): { x: number; y: number } | null {
    const directions = [
      { x: 1, y: 0 },  // right
      { x: -1, y: 0 }, // left
      { x: 0, y: 1 },  // down
    ...
    let queue: { x: number; y: number; }[] = [{ x: this.currentPos.x, y: this.currentPos.y }];
    let visited: boolean[][] = Array.from({ length: gameMap.length }, () => Array(gameMap[0].length).fill(false));
    ...
    while (queue.length > 0) {
      ...
      for (const dir of directions) {
        ...
        if (
          nextX >= 0 && nextX < gameMap[0].length &&
  ...
  private shouldPickupBlock(cell: any): boolean {
    // Check if there's a block to pickup at the current position
    return (
      (cell.left.type === this.BLOCK &&
  ...
  private findNewExploreTarget(
    ...
    let queue: { x: number; y: number; }[] = [{ x: currentPos.x, y: currentPos.y }];
    let visited: boolean[][] = Array.from({ length: gameMap.length }, () => Array(gameMap[0].length).fill(false));
  ...
  public turn(cell: any, gameMap: number[][]): string {
